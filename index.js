...
const itemsByRarity = new Collection();
const itemsByName = new Collection();
const parser = /([PWH],){1}([+-][0-9]+[,]){3}([+-][0-9]+)/;
...
Reflect.defineProperty(characters, "roll", {
  ...
  value: function roll(
    ...
    for (let i = 0; i < numRolls; i++) {
      ...
      rolls.push(
        itemsByRarity.get(result)[
          Math.floor(Math.random() * itemsByRarity.get(result).length)
        ]
      );
    }
    return rolls;
...
client.once("ready", async () => {
  ...
  rarities.forEach((b) => itemsByRarity.set(b, new Array()));
  const i = await Items.findAll();
  i.forEach((b) => {
    if (b.rarity !== "XR") {
      itemsByRarity.get(b.rarity).push(b);
    }
    itemsByName.set(b.name, b);
  });
  ...
  console.log(
    `Time until next day: ${hours} hours, ${minutes} minutes, and ${seconds} seconds`
  );
...
client.on("interactionCreate", async (interaction) => {
  ...
  try {
    switch (command.data.name) {
      ...
      case "rpg":
      case "item":
        return command.execute(interaction, characters, itemsByName);
      case "remind":
      ...
      case "admin":
        ...
        switch (interaction.options.getSubcommand()) {
          ...
          case "give":
            ...
            if (!user)
              return interaction.reply(`That character doesn't exist.`);
            if (itemName === "gold") {
              user.gold += 1;
              await user.save();
